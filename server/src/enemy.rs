...
use crate::player_stats::{player_stats, BASE_EXP_PER_KILL};
...
pub fn spawn_enemies(ctx: &ReducerContext) -> Result<(), String> {
    ...
    for player in alive_players {
        ...
        let distance = ctx.rng().gen_range(ENEMY_SPAWN_RADIUS * 0.8..ENEMY_SPAWN_RADIUS);
        ...
        let enemy_type = match ctx.rng().gen_range(0..4) {
            0 => EnemyType::Basic,
            1 => EnemyType::Fast,
            2 => EnemyType::Tank,
            3 => EnemyType::Elite,
    ...
    for mut enemy in enemies.iter() {
        ...
        // Attack if close enough
            ...
            let can_attack = match enemy.last_attack_time {
                Some(last_attack) => {
                    let elapsed_ms = now_ts.duration_since(last_attack).unwrap().as_millis() as u64;
            ...
            if can_attack {
                ...
                let mut player_stats = players_stats.player_id().find(target_player_id)
                    .ok_or_else(|| "Player not found to apply damage effect.".to_string())?;

                let mut current_health = player_stats.health;
                let armor = player_stats.armor;

                // Calculate damage with armor reduction
                let damage = enemy.damage * (1.0 - armor.min(0.8)); // Cap armor at 80% reduction
                
                // Apply damage to player
                let mut player = target_player;
                current_health = (current_health - damage).max(0.0);
                player.last_hit_time = Some(now_ts);
                
                // Check for death
                if current_health <= 0.0 && !player.is_dead {
                    player.is_dead = true;
    ...
    if enemy.health <= 0.0 {
        ...
        if let player_id = ctx.sender {
            if let Some(mut player) = ctx.db.player().identity().find(player_id) {
                ...
                if let Err(e) = crate::add_experience(ctx, exp_gain) {
